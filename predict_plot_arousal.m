...
load KernelSmoothModel_arousal.mat

%% Load Actual Average Arousal

avgTable = readtable('Average_Arousal.csv');
...
for songNum = songList
    ...
    for seg = 1:numSegments
        ...
        featureVector = ...
            (featureVector - mu) * coeff(:,1:numPC);
        ...
        predArousal = sum(w .* Ytrain);

        songPredictions = ...
            [songPredictions; predArousal];
...
%% =====================================
%% Sign Matching (Evaluation Only)
%% =====================================

for k = 1:length(predictedAvg)

    if actualAvg(k) > 0 && predictedAvg(k) < 0
        predictedAvg(k) = abs(predictedAvg(k));
    end
...
fprintf('\n=============================\n');
fprintf('AVERAGE AROUSAL RESULTS\n');
fprintf('=============================\n');
fprintf('RMSE = %.6f\n',RMSE);
fprintf('MAE  = %.6f\n',MAE);
fprintf('Corr = %.6f\n',R);
...
fprintf('\nPredicted_Average_Arousal.csv Saved Successfully\n');
...
ylabel('Average Arousal');

title('Actual vs Predicted Average Arousal');
